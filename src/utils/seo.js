...
export function generateMetaDescription(text, pathname) {
  ...
  const defaults = {
    ...
    "/about/":
      "Meet the team behind ProGreen Clean. Family-run, fully insured cleaning specialists serving Epsom and Surrey for over 10 years.",
    "/services/":
  ...
  return (
    defaults[pathname] ||
    `Professional cleaning services in Epsom, Surrey. ${SITE_NAME} — fully insured, eco-friendly, trusted locally.`
  );
}
...
/**
 * @param {{ratingValue: string | number, reviewCount: number} | null} aggregateRating
 */
export function generateLocalBusinessSchema(aggregateRating = null) {
  const schema = {
